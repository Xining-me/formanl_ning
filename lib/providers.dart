...
class AuthState extends ChangeNotifier {
  ...
  Future<void> load() async {
    ...
    if (raw != null) {
      ...
      currentUser = AppUser(
        ...
        email: m['email'],
        followingUserIds: {...(m['following'] as List).map((e) => e as String)},
        favoritePostIds: {...(m['favorites'] as List).map((e) => e as String)},
      );
    }
    notifyListeners();
  }
  ...
  Future<bool> login({required String studentId, required String email}) async {
    ...
    AppUser base = demoUser;
    base.nickname = demoUser.nickname;
    currentUser = AppUser(
      id: base.id,
...
class PostStore extends ChangeNotifier {
  ...
  List<Post> announcementsTop10() {
    ...
    ann.sort((a, b) => (b.likes + b.replies + b.bookmarks).compareTo(a.likes + a.replies + a.bookmarks));
  ...
  Post? findById(String id) => _posts.firstWhere((e) => e.id == id, orElse: () => null as Post);
